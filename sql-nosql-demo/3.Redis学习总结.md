...
[基于Redis分布式锁的正确打开方式](https://www.cnblogs.com/spec-dog/p/13320930.html)
...
[Redis5 压力测试结果反馈报告](https://www.cnblogs.com/netcore3/p/14121579.html)
### Redis面试题
...
上传redis文件到服务器目录下，解压移动到/usr/local/redis目录下，查看是否安装了gcc,# gcc -v
    进入redis目录，先make，再make install,接着make test，报错没有安装tcl:# yum install tcl
    出现绿色--- \o/ All tests passed without errors! 就好了
    其他的错误可以参考上面的参考资料
2，Redis启动
    1.在redis目录下有一个redis.conf文件，创建etc目录，复制到etc目录  # cp redis.conf etc/
        /usr/local/redis/etc/redis.conf作为redis启动的配置文件，
    2.修改配置文件：vi /usr/local/redis/etc/redis.conf   
        1.68行，注释掉 bind 127.0.0.1    =>  # bind 127.0.0.1    
        2.87行，修改  protected-mode yes => protected-mode no
        3.91行，修改你需要的端口默认6379   port 6379
        4.224行，修改允许后台启动 daemonize no => daemonize yes
        保存退出
    3.配置开机启动：vi /etc/rc.local  
        添加一行命令，允许开机启动
        /usr/local/redis/src/redis-server /usr/local/redis/etc/redis.conf
    4.redis服务器指定配置文件启动 /usr/local/redis/src/redis-server /usr/local/redis/etc/redis.conf
        redis客户端连接(交互式)#  redis-cli
        ...
        (如果没有-h参数，那么默认连接127.0.0.1；如果没有-p，那么默认6379端口，也就是说如果-h和-p都没写就是连接 127.0.0.1：6379这个Redis实例)
        停止Redis服务 redis-cli shutdown 可以在后面指定一个参数，nosave|save表示在关闭前是否生产持久化文件
    5.停止redis服务，查看端口：ps -ef|grep redis
                    关闭端口：kill -9  端口号
      执行命令重启： /usr/local/redis/src/redis-server /usr/local/redis/etc/redis.conf            
```
### 2.基于阿里云和Docker搭建Redis集群 [【查看集群配置文件模板】](https://github.com/peng4444/demo2020/blob/master/sql-nosql-demo/redis-docker%E9%9B%86%E7%BE%A4%E9%85%8D%E7%BD%AE%E6%96%87%E4%BB%B6.conf)
...
```markdown
目标：
    在linux主机上，使用Docker搭建redis cluster集群（含主从），使用本地java访问redis集群
说明：
    许多的教程都是创建docker内部网络，默认是bridge网络，在服务器上搭建的集群远程无法访问。
    host网络模式可以让容器ip、端口共享宿主机的网络，远程可以访问。【在自己的windows上使用docker搭建集群，本机代码好像可以访问】
    查看端口占用情况：  netstat -apn | grep 7000
    查看端口开放情况：  netstat -tlunp
操作环境
    使用一台Centos7 linux主机，使用Finall Shell远程连接，**已经安装docker**
1.拉取redis镜像和创建网络
    $ docker pull redis  -- 获取redis镜像
    $ docker network create redis-cluster-net  -- 创建redis集群网络，用于集群节点间通信
    $ docker network inspect redis-cluster-net  -- 查看创建的网络信息
2.在 /home/pbj/docker/redis-cluster目录下,准备一份模板文件redis-cluster.tmpl方便脚本生成多份配置，
    查看我的配置文件，cluster-announce-ip需要修改为你的服务器ip【或者公网IP】。主要配置如下：
    protected-mode no                         ## 87行
    port ${PORT}                              ## 91行
    appendonly yes                            ## 1089行
    cluster-enabled yes                       ## 1222行
    cluster-config-file nodes.conf            ## 1230行
    cluster-node-timeout 15000                ## 1236行
    cluster-announce-ip 公网IP                 ## 1373行，可以访问的IP地址
    cluster-announce-port ${PORT}             ## 1374行
    cluster-announce-bus-port 1${PORT}        ## 1375行
3.脚本生成6份配置，在 /home/pbj/docker/redis-cluster目录下
    $ for port in `seq 7000 7005`; do \
      mkdir -p ./${port}/conf \
      && PORT=${port} envsubst < ./redis-cluster.tmpl > ./${port}/conf/redis.conf \
      && mkdir -p ./${port}/data; \
    done
4.脚本启动6个容器，在 /home/pbj/docker/redis-cluster目录下
    $ for port in `seq 7000 7005`; do \
      docker run -d -ti -p ${port}:${port} -p 1${port}:1${port} --name redis-${port} \
      -v /home/pbj/docker/redis-cluster/${port}/conf/redis.conf:/usr/local/etc/redis/redis.conf \
      -v /home/pbj/docker/redis-cluster/${port}/data:/data \
      --restart always  --net redis-cluster-net \
      --sysctl net.core.somaxconn=1024 redis redis-server /usr/local/etc/redis/redis.conf; \
    done
5.查看容器启动日志，检查是否正常启动【如果报错一般是自己的配置文件错误】
    # 查看docker容器启动情况
    $ docker ps 
    # 查看是否有报错出现
    $ docker container logs redis-7000   
6.使用redis-cli命令行创建集群
    任意进入一个创建的Redis容器（redis-7000为容器names）
    $ docker exec -it redis-7000  /bin/bash
    # redis-cli --cluster create 公网ip:7000 公网ip:7001 公网ip:7002 公网ip:7003 公网ip:7004 公网ip:7005 --cluster-replicas 1
        会让你输入yes,开始初始化集群槽
        参数 --cluster-replicas 1，参数后面的数字表示的是主从比例，比如这里的 1 表示的是主从比例是 1:1
    $ redis-cli -c -p 7000 cluster info  # 查看redis集群信息
    $ redis-cli -c -p 7000 cluster nodes # 查看redis节点信息
    3e7e9055d499430543baa0c4dfe6db76f950139b 公网ip:7002@17002 master - 0 1607780266000 3 connected 10923-16383
    ad1d2c2777ac3e3d9681ae1eae391e040dbb6e5a 公网ip:7004@17004 slave 3e7e9055d499430543baa0c4dfe6db76f950139b 0 1607780269023 3 connected
    5e0700cdfaf8594783f23147e92a1f33e7be19f9 公网ip:7005@17005 slave d5417fa772a0b698e17c3806e758f55b5a4668dd 0 1607780265000 1 connected
    d8547eef504a7d03914c4ce7043856110cf2affd 公网ip:7003@17003 slave bd1bad327ed734f50d05ff0fd014af559b7d9124 0 1607780268020 2 connected
    bd1bad327ed734f50d05ff0fd014af559b7d9124 公网ip:7001@17001 master - 0 1607780267018 2 connected 5461-10922
    d5417fa772a0b698e17c3806e758f55b5a4668dd 公网ip:7000@17000 myself,master - 0 1607780267000 1 connected 0-5460
    $ redis-cli -c -p 7000 cluster slots # 查看redis集群槽信息，槽都在master节点上
7.检查集群数据插入查询情况
    $ redis-cli -c -p 7000 【-p:redis启动的端口，-c集群模式启动，不然无法读取到集群中其他节点的数据】[redis集群报错：(error) MOVED 解决方法](https://blog.csdn.net/csdn_cnmb/article/details/103034978)
    127.0.0.1:7000> set test 1234
    -> Redirected to slot [6918] located at 192.168.101.206:7001
    OK
    公网ip:7001> get test
    "1234"
    公网ip:7001> set hello redis
    -> Redirected to slot [866] located at 192.168.101.206:7000
    OK
    公网ip:7000> exit
    $ redis-cli -c -p 7003   # 登录到slave节点查询数据
    127.0.0.1:7003> get test
    -> Redirected to slot [6918] located at 192.168.101.206:7001
    "1234"
    公网ip:7001> 
8.容器启动，关闭，删除命令
    $ docker start redis-7000 redis-7001 redis-7002 redis-7003 redis-7004 redis-7005
...
9.远程访问
    包括使用redis远程访问工具，和Java代码访问都需要开放访问端口7000~7005和17000~17005
    查看端口开放情况：  netstat -tlunp
    使用阿里云服务器的需要添加安全组。【如果防火墙开启，防火墙同样开放端口访问。 】  
    测试端口开放情况：telnet 公网IP 端口 
其他：
    ...
    4.[docker发现端口是tcp6的 导致无法访问前端](https://blog.csdn.net/weixin_30235225/article/details/98672437)
    5.[阿里云轻量服务器 nginx docker 只监听ipv6 导致无法访问](https://blog.csdn.net/qq_43031691/article/details/90090071)
      [Docker映射端口时仅有IPv6无法到IPv4解决方法](https://blog.csdn.net/weixin_42842069/article/details/108640034)
```
### 3.基于阿里云和Docker配置Redis哨兵Sentinel模式
[基于Docker部署Redis主从复制及哨兵模式](https://blog.csdn.net/u012002805/article/details/107859437)
[一文掌握Redis的三种集群方案](https://www.cnblogs.com/spec-dog/p/12501895.html)
[Spring Boot（十三）：整合Redis哨兵，集群模式实践](https://www.cnblogs.com/spec-dog/p/12572120.html)
```markdown
Redis-Sentinel是Redis官方推荐的高可用性(HA)解决方案，当用Redis做Master-slave的高可用方案时，假如master宕机了，
    Redis本身(包括它的很多客户端)都没有实现自动进行主备切换，而Redis-sentinel本身也是一个独立运行的进程，
    它能监控多个master-slave集群，发现master宕机后能进行自动切换。
    Sentinel由一个或多个Sentinel实例组成的Sentinel系统可以监视任意多个主服务器，以及这些主服务器属下的所有从服务器，
    并在被监视的主服务器进入下线状态时，自动将下线主服务器属下的某个从服务器升级为新的主服务器。 
```
### Redis集群读写分离
[springboot中实现redis读写分离](https://blog.csdn.net/xue15029240296/article/details/96857052)
[redis实战--redis主从复制实现读写分离（原理）](https://blog.csdn.net/xuxian6823091/article/details/81195231)
```markdown
只有master可以进行write, 其他的没有write的权限，只有read权限，当master故障后，会选举一个出新的master, 
    并且哨兵会把新的master地址返回给项目，以便项目快速切换新的master，如下图就是切换后的架构。
```
### 4.Redis一些终端命令
