...
```
### 18.Redis跳跃表实现，为什么用skipList而不用红黑树【5+】
...
```
### 19.redis的zset怎么实现的？（跳表）那跳表是用什么判断每隔几个选一个呢？（随机算法）【3+】
```markdown
...
```
### 21.分布式缓存和本地缓存有啥区别？让你自己设计本地缓存怎么设计？如何解决缓存过期问题？如何解决内存溢出问题？
```markdown
...
```
### 23.redis数据分布方式？有什么优点？一致性hash呢？【2+】
```markdown
Hash：
​   客户端分片：哈希+取余
...
```
### Redis多线程
[Redis多线程原理详解](https://www.cnblogs.com/hlxs/p/14037919.html)
```markdown

```
### 25.Redis布隆过滤器原理与实现【5+】
```markdown
布隆过滤器:
...
[面试官：Redis 主从复制时网络开小差了怎么整？](https://www.cnblogs.com/caoyier/p/13984724.html)
[随笔分类 - redis、分布式锁的三种方式](https://www.cnblogs.com/ft535535/category/1366532.html)
[Redis实现缓存与分布式锁](https://www.cnblogs.com/chengming104/p/13857490.html)
...
[**关于Redis分布式锁这一篇应该是讲的最好的**](https://www.cnblogs.com/lwh1019/p/14115256.html)
...
[Redis配置文件(翻译)](https://blog.csdn.net/weixin_37690143/article/details/96031957)
## Redis实战
### 0.redis安装启动
...
```markdown
1.Redis安装
上传redis文件到服务器目录下，解压移动到/usr/local/redis目录下，查看是否安装了gcc,# gcc -v
    进入redis目录，先make，在make test，报错没有安装tcl:# yum install tcl
    出现绿色--- \o/ All tests passed without errors! 就好了
    其他的错误可以参考上面的参考资料
2，Redis启动
    在redis目录下有一个redis.conf文件，创建etc目录，复制到etc目录  # cp redis.conf etc/
...
```
### 0.基于Docker搭建Redis集群
[使用docker搭建redis集群](https://blog.csdn.net/shsongtao/article/details/87979855)
[【Redis自学笔记六】Docker搭建Redis集群，主从复制和哨兵模式](https://blog.csdn.net/weixin_45901207/article/details/107479478)
[基于Docker搭建Redis集群（主从集群）](https://www.cnblogs.com/niceyoo/p/14118146.html)
[基于Docker部署Redis主从复制及哨兵模式](https://blog.csdn.net/u012002805/article/details/107859437)
[-- [Redis] 手动搭建标准6节点Redis集群(docker)](https://www.cnblogs.com/slowbirdoflsh/p/11633113.html#_lab2_2_2)
```markdown
1.查看端口占用情况：  netstat -apn | grep 6379
2.拉取 Redis 镜像： docker pull redis:6.0.9
    # 查看docker网络 docker network ls
    # 创建属于redis的集群网络  docker network create redis-cluster-net
    # 查看集群网络信息 docker network inspect redis-cluster-net  -- 网关Gateway: 172.18.0.1
3.创建本地隐射文件和配置文件
    
4.创建6个Redis容器：
    docker create --name redis-6371 --net redis-cluster-net -p 6371:6371 -p 16371:16371 \
        -v /usr/local/docker/redis/6371/redis.conf:/etc/redis/redis.conf \
        -v /usr/local/docker/redis/6371/data/:/data/redis/ \
        redis:6.0.9 \
        redis-server /etc/redis/redis.conf; 
    docker create --name redis-6372 --net redis-cluster-net -p 6372:6372 -p 16372:16372 \
        -v /usr/local/docker/redis/6372/redis.conf:/etc/redis/redis.conf \
        -v /usr/local/docker/redis/6372/data/:/data/redis/ \
        redis:6.0.9 \
        redis-server /etc/redis/redis.conf;
    docker create --name redis-6373 --net redis-cluster-net -p 6373:6373 -p 16373:16373 \
        -v /usr/local/docker/redis/6373/redis.conf:/etc/redis/redis.conf \
        -v /usr/local/docker/redis/6373/data/:/data/redis/ \
        redis:6.0.9 \
        redis-server /etc/redis/redis.conf;
    docker create --name redis-6374 --net redis-cluster-net -p 6374:6374 -p 16374:16374 \
        -v /usr/local/docker/redis/6374/redis.conf:/etc/redis/redis.conf \
        -v /usr/local/docker/redis/6374/data/:/data/redis/ \
        redis:6.0.9 \
        redis-server /etc/redis/redis.conf;
    docker create --name redis-6375 --net redis-cluster-net -p 6375:6375 -p 16375:16375 \
        -v /usr/local/docker/redis/6375/redis.conf:/etc/redis/redis.conf \
        -v /usr/local/docker/redis/6375/data/:/data/redis/ \
        redis:6.0.9 \
        redis-server /etc/redis/redis.conf;
    docker create --name redis-6376 --net redis-cluster-net -p 6376:6376 -p 16376:16376 \
        -v /usr/local/docker/redis/6376/redis.conf:/etc/redis/redis.conf \
        -v /usr/local/docker/redis/6376/data/:/data/redis/ \
        redis:6.0.9 \
        redis-server /etc/redis/redis.conf;
5.启动 Redis 容器
    docker start redis-6371 redis-6372 redis-6373 redis-6374 redis-6375 redis-6376
    docker stop redis-6371 redis-6372 redis-6373 redis-6374 redis-6375 redis-6376
    docker container rm redis-6371 redis-6372 redis-6373 redis-6374 redis-6375 redis-6376
5.组建 Redis 集群
    docker启动之后，docker network inspect redis-cluster-net命令查看为每个节点分配的ip
    执行组件集群的命令：# 组建集群,172.18.0.2为docker网络的ip地址
    # redis-cli --cluster create 172.18.0.2:6371 172.18.0.3:6372 172.18.0.4:6373 172.18.0.5:6374 172.18.0.6:6375 172.18.0.7:6376 --cluster-replicas 1
    参数 --cluster-replicas 1，参数后面的数字表示的是主从比例，比如这里的 1 表示的是主从比例是 1:1
    1.报错[ERR] Node 192.168.101.206:6371 is not empty. Either the node already knows other nodes (check with CLUSTER NODES) or contains some key in 
        把之前创建的docker容器启动产生的文件删除
    2.[centos7创建redis集群一直卡在Waiting for the cluster to join](https://blog.csdn.net/qq_37481017/article/details/86002454) 
    【关闭防火墙不是解决方式，需要找到其他的解决方式】
    创建成功后，通过 redis-cli 查看一下集群节点信息：【docker上的Redis映射到了服务器上，可以直接在服务器上执行命令】
    # redis-cli -p 6371 -c  【-p:redis启动的端口，-c集群模式启动，不然无法读取到集群中其他节点的数据】[redis集群报错：(error) MOVED 解决方法](https://blog.csdn.net/csdn_cnmb/article/details/103034978)
    127.0.0.1:6371> cluster nodes
    382e20438f448ac984eba8c7bcb00a71fbd2b3f6 172.17.0.5:6372@16372 myself,master - 0 1607673121000 2 connected 5461-10922
    ce71e22cfd59658629bab92558c8f7f398b444af 172.17.0.1:6371@16371 master - 0 1607673122626 1 connected 0-5460
    3aa48993012f9efc8016db8792422a2f88415318 172.17.0.1:6375@16375 slave ce71e22cfd59658629bab92558c8f7f398b444af 0 1607673123000 1 connected
    c4c970a6367739140740848782f21f89aec74311 172.17.0.1:6374@16374 slave 52b6df313355278969a93d6ac1509a6225a973f8 0 1607673121624 3 connected
    2d7360f0a67da8c6b7236d0711d7e231fab9cc20 172.17.0.1:6376@16376 slave 382e20438f448ac984eba8c7bcb00a71fbd2b3f6 0 1607673124634 2 connected
    52b6df313355278969a93d6ac1509a6225a973f8 172.17.0.1:6373@16373 master - 0 1607673124000 3 connected 10923-16383
    127.0.0.1:6371> set hello redis  -- 在6371节点上设置数据
    OK
    127.0.0.1:6371> get hello
    "redis"
    172.17.0.1:6375> get hello       -- 在6375节点上读取，并且6375会重定向到6371
    -> Redirected to slot [866] located at 172.17.0.1:6371
    "redis"
    172.17.0.1:6371>
```
[[Redis] 使用redis-cli命令搭建标准6节点Redis集群(docker)](https://www.cnblogs.com/slowbirdoflsh/p/11636120.html)



[[Demo示例]Docker搭建redis cluster集群，外网可访问](https://blog.csdn.net/haerxiong/article/details/85064502)
[关于docker 构建redis 主从/哨兵/集群网络问题](https://blog.csdn.net/qq_14869093/article/details/102453390)
```markdown
脚本生成6份配置
...
脚本启动6个容器
    for port in `seq 7000 7005`; do \
      docker run -d -ti \
      --privileged=true -v /usr/local/docker/redis/${port}/conf/redis.conf:/usr/local/etc/redis/redis.conf \
      --privileged=true -v /usr/local/docker/redis/${port}/data:/data \
      --restart always --name redis-${port} --net host \
      --sysctl net.core.somaxconn=1024 redis:6.0.9 redis-server /usr/local/etc/redis/redis.conf; \
    done

docker container logs redis-7000

使用redis-cli命令行创建集群【前提本地需要安装有相同版本的redis】
redis-cli --cluster create 192.168.101.206:7000 192.168.101.206:7001 192.168.101.206:7002 192.168.101.206:7003 192.168.101.206:7004 192.168.101.206:7005 --cluster-replicas 1
redis-cli -c -p 7000 cluster info
redis-cli -c -p 7000 cluster nodes
    3e7e9055d499430543baa0c4dfe6db76f950139b 192.168.101.206:7002@17002 master - 0 1607780266000 3 connected 10923-16383
    ad1d2c2777ac3e3d9681ae1eae391e040dbb6e5a 192.168.101.206:7004@17004 slave 3e7e9055d499430543baa0c4dfe6db76f950139b 0 1607780269023 3 connected
    5e0700cdfaf8594783f23147e92a1f33e7be19f9 192.168.101.206:7005@17005 slave d5417fa772a0b698e17c3806e758f55b5a4668dd 0 1607780265000 1 connected
    d8547eef504a7d03914c4ce7043856110cf2affd 192.168.101.206:7003@17003 slave bd1bad327ed734f50d05ff0fd014af559b7d9124 0 1607780268020 2 connected
    bd1bad327ed734f50d05ff0fd014af559b7d9124 192.168.101.206:7001@17001 master - 0 1607780267018 2 connected 5461-10922
    d5417fa772a0b698e17c3806e758f55b5a4668dd 192.168.101.206:7000@17000 myself,master - 0 1607780267000 1 connected 0-5460
redis-cli -c -p 7000 cluster slots
redis-cli -c -p 7000
    127.0.0.1:7000> set test 1234
    -> Redirected to slot [6918] located at 192.168.101.206:7001
    OK
    192.168.101.206:7001> get test
    "1234"
    192.168.101.206:7001> set hello redis
    -> Redirected to slot [866] located at 192.168.101.206:7000
    OK
    192.168.101.206:7000> 
redis-cli -c -p 7003
    127.0.0.1:7003> get test
    -> Redirected to slot [6918] located at 192.168.101.206:7001
    "1234"
    192.168.101.206:7001> 


redis-cli --cluster create 47.115.91.92:7000 47.115.91.92:7001 47.115.91.92:7002 47.115.91.92:7003 47.115.91.92:7004 47.115.91.92:7005 --cluster-replicas 1


redis-cli --cluster create 47.115.91.92:7000 47.115.91.92:7001 47.115.91.92:7002 47.115.91.92:7003 47.115.91.92:7004 47.115.91.92:7005 --cluster-replicas 1

docker stop redis-7000 redis-7001 redis-7002 redis-7003 redis-7004 redis-7005
docker container rm redis-7000 redis-7001 redis-7002 redis-7003 redis-7004 redis-7005


阿里云访问添加安全组，本地服务器开放端口访问    
```
### Redis一些终端命令
...
```
### springboot整合redis
[**cn/pbj/demo202/redis/查看代码**--springboot整合redis集群详解](https://blog.csdn.net/hp_yangpeng/article/details/89002788)
### 
