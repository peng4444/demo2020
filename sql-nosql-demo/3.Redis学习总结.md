...
5.组建 Redis 集群
    执行组件集群的命令：# 组建集群,192.168.101.206为当前物理机的ip地址
    # redis-cli --cluster create 192.168.101.206:6371 192.168.101.206:6372 192.168.101.206:6373 192.168.101.206:6374 192.168.101.206:6375 192.168.101.206:6376 --cluster-replicas 1
    参数 --cluster-replicas 1，参数后面的数字表示的是主从比例，比如这里的 1 表示的是主从比例是 1:1
    [centos7创建redis集群一直卡在Waiting for the cluster to join](https://blog.csdn.net/qq_37481017/article/details/86002454) 
