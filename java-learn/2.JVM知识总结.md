...
 Execution Engine执行引擎：Class Loader只管加载，只要符合文件结构就加载，至于说能不能运行，则不是它负责的，
    是由Execution Engine负责的。执行引擎也叫做解释器 (Interpreter)，负责解释命令，提交操作系统执行。
...
4.堆内存(线程共享):主要用于存放新创建的对象(所有对象都在这里分配内存)，是垃圾收集的主要区域（"GC堆"），对象实例的分配区域。
        1.Java堆内存 = 老年代 + 新生代，新生代与老年代默认比例的值为 1:2 ，可以通过参数 –XX:NewRatio配置。
...
虚拟机栈：每个Java方法在执行的同时会创建一个栈帧用于存储局部变量表、操作数栈、常量池引用等等信息。
    从方法调用直至执行完成的过程，就对应着一个栈帧在Java虚拟机中入栈和出栈的过程。
...
```
#### 1.3.Java中成员变量、局部变量、静态变量、常量分别存储在那些内存区域中？【2+】
```markdown
 成员变量：堆内存
 局部变量：
...
![image-20201202110124728](https://raw.githubusercontent.com/peng4444/picgo/main/img/20201202110131.png)
```markdown
对象在内存中的组成部分（对象在内存的划分、对象的内存布局）:
     对象头(Object Header)：（对象头占用12个字节，存储内容包括对象标记和类元信息）
         第一：对象标记（markOop）：存储对象自身的运行时数据，如哈希码、GC分代年龄、锁标志、偏向线程ID等，这部分占8B，称为Mark Word。
         第二：类元信息（klassOop）：是对象指向它的类型元数据的指针，占4B。JVM通过该指针来确定对象是哪个类的实例。
...
**回收不可达的对象是不是一定就要被回收掉？？**
    ...
    当对象没有覆盖finalize()⽅法或者已被虚拟机调⽤过，那么就认为是没必要的。如果该对象有必要执⾏finalize()⽅法，那么这个对象将会放在⼀个称为F-Queue的对队列中，虚拟机会触发⼀个Finalize()线程去执⾏，
    此线程是低优先级的，并且虚拟机不会承诺⼀直等待它运⾏完，这是因为如果finalize()执⾏缓慢或者发⽣了死锁，那么就会造成F-Queue队列⼀直等待，造成了内存回收系统的崩溃。GC对处于F-Queue中的对象进⾏第⼆次被标记，这时，该对象将被移除”即将回收”集合，等待回收。
```
### 5.JVM垃圾回收算法【10+】
...
Parallel Scavenge收集器:(新生代收集器、复制算法、并行垃圾回收器)  
...
**CMS收集器**:(老年代收集器、标记-清除算法，并发垃圾回收器) JVM参数: -XX:+UseConcMarkSweepGC
    ...
    **初始标记和重新标记需要STW**（Stop The World，系统停顿）。
    [垃圾回收器为什么必须要停顿下？](https://www.cnblogs.com/jiagoujishu/p/13788291.html)
    优点:并发收集，低停顿
    ...
    [三色标记法与读写屏障](https://www.cnblogs.com/gangtie/p/13877833.html)
**G1收集器**:(混合收集器 复制+标记-整理算法) JVM参数: -XX:+UseG1GC
    ...
    G1 收集器的运作大致可划分为以下几个步骤：
        ...
        2.并发标记Concurrent Marking，在整个堆中进行并发标记(和应用程序并发执行)，此过程可能被young GC中断。
            在并发标记阶段，若发现区域对象中的所有对象都是垃圾，那个这个区域会被立即回收(图中打X)。同时，并发标记过程中，会计算每个区域的对象活性(区域中存活对象的比例)。
        3.最终标记Remark：会有短暂停顿(STW)。为了修正在并发标记期间因用户程序继续运作而导致标记产生变动的那一部分标记记录，虚拟机将这段时间对象变化记录在线程的
            Remembered Set Logs里面，最终标记阶段需要把Remembered Set Logs的数据合并到Remembered Set中。这阶段需要停顿线程，但是可并行执行。
        4.筛选回收：多线程清除失活对象，会有STW。首先对各个Region中的回收价值和成本进行排序，根据用户所期望的GC停顿时间来制定回收计划。
...
ZGC(Z Garbage Collector): 
    [ ZGC 的特性，或者说几个关键点，然后再简述下整体回收流程。](https://www.cnblogs.com/yescode/p/13997822.html)
    ...
    ZGC垃圾回收的步骤：（**只有初始标记、再标记、初始转移阶段是STW的**）
        - 初始标记：就扫描GC Roots直接可达的，耗时很短，重新标记一般而言也很短，如果超过1ms会再次进入并发标记阶段再来一遍，所以影响不大。
        - 并发标记：就是根据初始标记的对象开始并发遍历对象图，还会统计每个region的存活对象的数量。
        - 再标记，非强引用根标记：这一阶段是STW的，因为并发阶段应用线程还是在运行的，所以会修改对象的引用导致漏标的情况。因此需要个再标记阶段来标记漏标的那些对象。
            如果这个阶段执行的时间过长，就会再次进入到并发标记阶段，因为ZGC的目标就是低延迟，所以一有高延迟的苗头就得扼制。
        - 并发转移准备：包括非强引用并发标记、重置转移集、回收无效页面、选择待回收页面、初始化转移表步骤，这几个步骤都是并发的。
            - 非强引用并发标记和引用并发处理：就是上一步非强根的遍历，然后引用就软引用、弱引用、虚引用的一些处理。这个阶段是并发的。
            - 重置转移集：对那些转移分区的地址映射集做一个复位的操作。
            - 回收无效分区：回收那些物理内存已经被释放的无效的虚拟内存页面
            - 选择待回收页面：这和G1一样，因为会有很多可以回收的分区，会筛选垃圾较多的分区，来作为这次回收的分区集合。
            - 初始化待转移集合的转移表：这一步就是初始化待回收的分区的 forwardingTable。
        - 初始转移：从根集合出发，如果对象在转移的分区集合中，则在新的分区分配对象空间。如果不在转移分区集合中，则将对象标记为 Remapped。初始转移阶段也是扫描GC Roots也很短，所以可以认为ZGC几乎是并发的。
        - 并发转移：从上一步转移的对象开始遍历，做并发转移。
...
而Full GC则相对复杂，有以下条件：
    ...
    6.Metaspace（元空间）在空间不足时会进行扩容，当扩容到了-XX:MetaspaceSize参数的指定值时，也会触发FGC。
...
避免：
     不要在循环中创建对象
     不要一次调用过多数据
     大量字符串使用StringBuffer或者StringBuilder
...
[对Java中SPI的理解吧](https://www.cnblogs.com/jimoer/p/14095489.html)
```
#### 5.自定义类加载器实现【2+】
...
```
#### 8.了解下tomcat的类加载机制
[Tomcat 第六篇：类加载机制](https://www.cnblogs.com/babycomeon/p/13783984.html)
```markdown
步骤：
...
内存模型三大特性
    原子性：一个操作要么执行要么不执行，执行期间不会被其他线程打断。
    可见性：一个线程改变了变量的值，对其他线程来说是可见的
    有序性：代码执行的先后顺序，因为可能发生指令重排序，所以执行的顺序不一定就是编写的顺序
```
### 11.JVM参数及JVM启动参数【5+】
...
[Class常量池、运行时常量池、字符串常量池](https://www.cnblogs.com/danzZ/p/14122466.html)
```markdown
Java中有三个常量池：字符串常量池、运行时常量池、class常量池
     字符常量池：全局字符串池里的内容是在类加载完成，经过验证，准备阶段之后在堆中生成字符串对象实例，也就是编译期间字符常量池已经创建完成。
     class文件中除了包含类的版本、字段、方法、接口等描述信息外，还有一项信息就是常量池(constant pool table)——注意这里的class常量池是存放在class文件中，
        而不是在jvm中，用于存放编译器生成的各种字面量(Literal)和符号引用(SymbolicReferences)。
        字面量就是我们所说的常量概念，符号引用就是在类加载阶段的解析阶段，把符号引用替换成直接引用。
     jvm在执行某个类的时候，必须经过加载、连接、初始化，而连接又包括验证、准备、解析三个阶段。
        而当类加载到内存中后，jvm就会将class常量池中的内容存放到运行时常量池中，运行时常量池也是每个类都有一个。
     关于class常量池和运行时常量池可以这么理解：在java文件编译时，类的字面量和符号引用，会存放在class类常量池，但是当类文件加载到jvm之后，class常量池的内容就被会加载到运行时常量池。
常量池的位置
...
```
### 16.关于强引用、软引用、弱引用、虚引用【5+】
[强引用、软引用、弱引用、虚引用](https://www.cnblogs.com/cxuanBlog/p/12774370.html)|
...
```
### 17.JAVA各种OOM代码样例及解决方法【5+】
[JAVA各种OOM代码样例及解决方法](https://www.cnblogs.com/huangqingshi/p/13336648.html)
...
```
### 19.JVM性能调优监控工具jps、jstack、jmap、jhat、jstat、hprof使用详解【5+】
...
[Java性能调优命令](https://www.cnblogs.com/kongzhongqijing/tag/jvm/)
```markdown
jps虚拟机进程状况工具，主要用来输出JVM中运行的进程状态信息。
    语法格式如下：jps [options] [hostid]   命令行参数选项说明如下：
...
jinfo：Java配置信息工具，查看JVM参数
    ...
    jinfo 11666
    jinfo -flags 11666
jstack：Java堆栈跟踪工具，主要用来查看某个Java进程内的线程堆栈信息。查看JVM线程快照，jstack命令可以定位线程出现长时间卡顿的原因，例如死锁，死循环
...
jmap（Memory Map）Java内存映像工具和 jhat（Java Heap Analysis Tool）虚拟机堆转储快照分析工具：jmap导出堆内存，然后使用jhat来进行分析，jmap语法格式如下：
    jmap：可以用来查看内存信息
        ...
        16.-F 强制选项
jstat：虚拟机统计信息监视工具，（JVM统计监测工具）: 看看各个区内存和GC的情况,查看JVM运行时的状态信息，包括内存状态、垃圾回收.
...
### 
[字节码增强技术探](https://www.cnblogs.com/meituantech/p/11497262.html)
