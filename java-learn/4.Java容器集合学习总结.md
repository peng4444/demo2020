# Java容器集合总结及面试题
[参考博客：不可错过的java面试博客之java集合篇](https://www.cnblogs.com/jichi/p/12834238.html)
[参考资料：Java集合大全](https://www.cnblogs.com/wangyongwen/p/13661981.html)
    ...
    Map定位元素的时间复杂度优化到O(1)
        AbsractMap：（定义了普通的Map集合具有的通用行为）
            HashMap：（数组+链表+红黑树，非线程安全，JDK1.8之前HashMap由数组+链表组成的）
                ...
                Poperties
            HashTable：（存储结构是数组+链表，线程安全(synchronized锁)），（默认长度为11，负载因子为0.75F，扩容为原长度2倍+1）
...
```
### 4.Collection和Collections有什么区别？【5+】
```markdown
...
1.List是无序的Collection，List一共三个实现类：分别是ArrayList、Vector和LinkedList
    1.1：ArrayList通过**数组**实现的，允许对元素进行快速随机访问。缺点是每个元素之间不能有间隔，当数组大小不满足时需要增加存储能力，
        ...
        适合随机查找和遍历，不适合插入和删除。排列有序，可重复，容量不够的时候，当前容量的1.5倍。(默认容量为10)。
...
##### ArrayList简介
```markdown
ArrayList在底层使用的是数组的数据结构，所以ArrayList的增加和删除的时间复杂度是受增加和删除的位置决定的。
但是ArrayList的查询效率很高，因为是在内存空间上是连续的，所以直接通过索引就能找到相应的位置。
ArrayList增加和删除的时间复杂度是O(n)，查找的时间复杂度是O(1)。
```
##### ArrayList的扩容机制
```markdown
1：ArrayList无参构造器初始化时，默认大小是空数组，并不是大家常说的10，10是在第一次add的时候扩容的数组值。
    在**真正进行第一次add操作的时候才分配空间**。
2.当里面的元素个数随着操作到达10个的时候开始扩容，每次扩容后的新数组大小是原来的1.5倍，并且将原来数组中元素一个一个复制到新数组的对应位置上。
3.ArrayList中的数组的最大值是Integer.MAX_VALUE，超过这个值，JVM就不会给数组分配内存空间了。
4.扩容完成之后，赋值是非常简单的，直接往数组上添加元素即可：elementData[size++]= e。所以这里的操作是线程不安全的。
5.扩容是Arrays.copyOf(elementData, newCapacity);来实现的，扩容是会先新建一个符合我们预期容量的新数组，然后把老数组的数据拷贝过去。
6.数组初始化，被加入一个值后，如果我使用addAll方法，一下子加入15个值，那么最终数组的大小是多少？
...
>> 只有当ArrayList作为共享变量时，才会有线程安全问题，当ArrayList是方法内的局部变量时，是没有线程安全的问题的。
```java
// ArrayList有线程安全问题的本质，是因为ArrayList自身的elementData、size、modConut在进行各种操作时，都没有加锁，
// 而且这些变量的类型并非是可见（volatile）的，所以如果多个线程对这些变量进行操作时，可能会有值被覆盖的情况。
// 推荐使用Collections#synchronizedList来保证线程安全，SynchronizedList是通过在每个方法上面加上锁来实现，虽然实现了线程安全，但是性能大大降低
...
### 4 Set
#### 4.1.Set
[Set 集合](https://www.cnblogs.com/chuonye/p/11260056.html)
```markdown
...
1.Map
    ...
    1.4：TreeMap（可排序）实现SortedMap接口，能够把它保存的记录根据键排序，默认是按键值的升序排序。
        TreeMap基于红黑树实现，增删改查的平均和最差时间复杂度均为O(logn)，最大特点是Key有序。Key必须实现Comparable接口或提供的Comparator比较器，所以Key不允许为null。
...
[TreeMap源码](https://www.cnblogs.com/chuonye/p/11245010.html)
### 6 Queue
```markdown
...
数据结构：
    哈希表结构（链表散列：数组+链表+红黑树）实现，结合数组和链表的优点。
        ...
        当链表长度减少到6时，退转为链表。有个差值7可以防止链表和树之间频繁的转换。
    原因：
...
（JDK1.7之前使用头插法、JDK1.8使用尾插法）（注意：当碰撞导致链表大于TREEIFY_THRESHOLD=8时，就把链表转换成红黑树，当等于6时退化为链表。）
```
...
1.7之前进行五次异或操作。。。我也不懂为什么要5次，可能是想充分的让每位都得到运用。然后与当前容量减1进行&操作，得到存放的桶的位置。
1.8中就只做了一次异或操作，将高16位和低16位进行异或操作，然后与当前容量减1进行&操作，得到桶的位置。
为什么要用异或运算符？
...
```
#### 7.HashMap中put、get方法的过程？【5+】
```markdown
put()方法实现：
    1.计算关于key的hashcode值（与Key.hashCode的高16位做异或运算）
    2.如果散列表为空时，调用resize()初始化散列表
    3.如果没有发生碰撞，直接添加元素到散列表中去（碰撞的意思是计算得到的Hash值相同，需要放到同一个bucket中）
...
```
#### 8.hashMap扩容机制，扩容resize()又是如何实现的？
```markdown
...
在Jdk1.7和Jdk1.8中，HashMap初始化这个容量的时机不同。
    1.7当size超过装载因子规定的容量的时候，发生扩容，HashMap的容量变为原来的两倍（方便用位运算进行hash运算），然后重新计算hash值，使用头插法重新装填到新的hashmap对应的桶上。
    1.8之后，计算新的对应的桶位置时直接将原来的hash值与新容量进行&操作，因为容量的二进制表示只有一位1，比如容量32的二进制为（100000），
        所以得到是0的直接保持在原地，得到是容量大小的，就直接移动到原位置加原大小的新位置上。
...
1.- Java7 HashMap
    ...
    内部是由一个Entry类型的数组来实现的，数组中的每个位置可以看成是一个桶，来存放hash计算后相应位置的键值对。
        当出现hash冲突的时候，也就是多个key映射到同一个桶的位置，就使用链地址法解决hash冲突。
...
3.- Java8 HashMap
    ...
    resize()方法用于初始化数组或数组扩容，每次扩容后，容量为原来的2倍，并进行数据迁移。
        将Entry改为了接口，用一个Node的内部类实现了Entry接口，其他的存储结构还是跟1.7保持一样。
        但与1.7之前改进的地方有如果当前桶存放的Entry，也就是键值对超过了8个，将转换位红黑树结构来保证查找的效率。
...
```
#### 2.ConcurrentHashMap锁机制具体分析（JDK1.7VSJDK1.8）？
```markdown
...
　　　　③、每个桶是由若干个HashEntry对象链接起来的链表
    即数据的为Segment数组下来再连接HashEntry数组，从底往上看也就是将桶分组，每个组由一个Segment来管理，当操作一个Segment下属的HashEntry时，会进行上锁操作，避免并发执行错误。
    不同于HashMap，HashEntry里面的数据（value和next指针）加上了volatile修饰，来保证在多线程并发的情况下不会拿到错误的数据。所以在使用get方法时，是不会加上锁的。
    但在使用put方法时，如果是修改的同一个Segment下面的hashEntry，那么就会对这一段的HashEntry进行上锁。
JDK1.8中，采用Node+CAS+Synchronized来保证并发安全。取消类Segment，直接用table数组存储键值对；
    当HashEntry对象组成的链表长度超过8时，链表转换为红黑树，提升性能。底层变更为数组+链表+红黑树。
    1.8之后，将Segment的思想取消，因为将Entry作为接口，那么它的实现Node也将value和next指针用volatile进行了修饰，这样get方法就不会读到错误的数据。
    在put方法中，如果当前的Node为空，那么就尝试使用cas的方式添加头节点。如果已经有节点了就直接使用**synchronized**进行添加链表节点的操纵。
    volatile V val;
    volatile Node<K,V> next;
    volatile Node<K,V>[] table
ConcurrentHashMap在JDK1.8中，为什么要使用内置锁synchronized来代替重入锁ReentrantLock？
    ①、粒度降低了；
...
但是这个数据在并发场景下是随时都在变的。
```
#### 8.为什么ConcurrentHashMap比HashTable效率要高？
```markdown
...
[HashMap 、ConcurrentHashMap知识点全解析](https://www.cnblogs.com/mengxinJ/p/13779516.html)
```markdown
我们从类注释上大概可以得到如下信息：
...
put 方法，采取了很多手段来保证了线程安全，是平时面试时的重中之重。
```
#### 10.computeIfAbsent和putIfAbsent方法的区别？
```markdown
1、ThreadLocalRandom，不能使用静态变量，因为在初始化的时候，通过Unsafe把seed和当前线程绑定了，在多线程情况下，只有主线程和seed绑定了，其他线程在获取seed的时候就是有问题的；
2、computeIfAbsent的value是接受一个Function，而putIfAbsent是是接受一个具体的value，不会对value处理。
```
### 9.怎么确保一个集合不能被修改？【5+】
```markdown
方式一：final：
...
```
#### [5.Java集合框架——大量数据处理操作练习题](https://blog.csdn.net/ZQ_313/article/details/84797467)
#### 集合去重
[对Java8中distinct()的思考](https://blog.csdn.net/puppylpg/article/details/78556730)
```markdown
【参考】利用Set元素唯一的特性，可以快速对一个集合进行去重操作，避免使用List的contains方法进行遍历、对比、去重操作。
开发中常见到使用 Set 去重的代码如下：注：Set自身可以保证不重复，不需要先通过contains判断不存在再添加元素。
...
集合List和Set去重性能比较：
    ...
    查看源码得：Set去重时间复杂度为O(n)，而List去重时间复杂度为O(n^2)
    发现当元素较少时两者耗时差距很小，随着元素的增多耗时差距越来越大。
为什么有人会用List的contains方法进行遍历、对比然后去重呢？
    - 基础不扎实，不了解这种操作的时间复杂度；
    - 为了维持返回值的类型。
    // 数据
...
```
#### 集合并集交集和差集
[《手册》详解 14节 集合去重的正确姿势学员并集差集问题解答](https://www.imooc.com/article/296110)
```markdown
1. 不推荐用Arrays#asList
    【强制】使用工具类Arrays.asList()把数组转换成集合时，不能使用其修改集合相关的方法，它的add/remove/clear方法会抛出UnsupportedOperationException异常。
2. 一个是 List求 差集和交集常见用方式；用guava包或者自定义工具类实现
