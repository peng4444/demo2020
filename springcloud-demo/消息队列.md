...
[分布式消息队列](https://www.cnblogs.com/lmhcblog/p/mq.html)
```markdown
...
```
### 04.安装部署RocketMQ
[Spring Cloud Alibaba学习笔记（9） - RocketMQ安装与RocketMQ控制台](https://www.cnblogs.com/fx-blog/p/11732749.html)
```markdown
...
2.从Apache RocketMQ的官网【http://rocketmq.apache.org/ 】可以进入RocketMQ的下载地址，下载Binary文件即可
> unzip rocketmq-all-4.6.1-bin-release.zip   # 解压缩
> mv rocketmq-all-4.6.1-bin-release /usr/local/rocketmq  # 重命名
> cd /usr/local
> cd rocketmq
> nohup sh bin/mqnamesrv &  # 启动Name Server
> tail -f ~/logs/rocketmqlogs/namesrv.log  # 验证是否启动成功
> nohup sh bin/mqbroker -n localhost:9876 &  # 启动Broker
> tail -f ~/logs/rocketmqlogs/broker.log # 验证是否启动成功
