...
[Centos创建用户并授权](https://blog.csdn.net/bug4pie/article/details/79761443)
```markdown
sudo // 切换到root用户
adduser 命令：添加新用户
    $ adduser pbj // 命令添加新用户
...
为新用户授权：
    ...
    # whereis sudoers  ## 查看sudoers文件在哪个目录
    # ls -l /etc/sudoers  ## 查看sudoers文件权限
    # chmod -v u+w /etc/sudoers  ## sudoers文件只有读权限，为其添加写权限
    # vi /etc/sudoers   ## 修改sudoers文件在root用户下面添加
        ## Allow root to run any commands anywher
        root  ALL=(ALL)    ALL
        pbj   ALL=(ALL)    ALL   ## 新增的新用户，可以验证密码使用sudo命令
    # chmod -v u-W /ect/sudoers  ## 收回sudoers文件的写权限
deluser：删除用户
